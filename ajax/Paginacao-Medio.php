...
if(isset($id)){
	...
	$sql_total = "SELECT id FROM curso  WHERE horario='medio'";
	...
	if($pagina<$totalpag){
		$setaRight = ('<div class="p4" onClick="paginacao.iniciar(\'.ajax-cursos\',\'Medio\',\''.$paginaMais.'\');">PRÓXIMO</div>');
	}else{
		$setaRight = ('<div class="p4"  style="cursor:default; opacity:0.5;">PRÓXIMO</div>');
	}
	if($pagina>1){
		$setaLeft = ('<div class="p3" onClick="paginacao.iniciar(\'.ajax-cursos\',\'Medio\',\''.$paginaMenos.'\');">ANTERIOR</div>');
	}else{
...
<div class="container-titulos">
	...
	<section class="containner">
			 <?php
                        ...
                        $sql = mysqli_query($conexao, "SELECT * FROM curso WHERE horario='medio' AND status='ativo' ORDER BY id DESC LIMIT $inicio, $quantidade");
                            $i = 0;
