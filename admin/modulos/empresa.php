...
$eu = $_SESSION["adm_id"];
$sql = mysqli_query($conexao, "SELECT * FROM empresa WHERE id='1'");
$item = mysqli_fetch_array($sql);
if($_POST){
$titulo = $_POST['titulo'];
$texto = $_POST['texto'];
echo "<script>alert('Dados atualizados!');</script>";
    mysqli_query($conexao, "UPDATE empresa SET titulo='$titulo', texto='$texto' WHERE id='1'");
    header('Refresh:0');
    //header('Location: /index.php?link=meus_dados');
}
?>
   <form method="post" enctype="multipart/form-data">
	<div id="bx_inicio">Editar Empresa</div>
