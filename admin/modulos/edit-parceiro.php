...
if($tipo == 'editar'){
  ...
  $foto = $_FILES['imagem'];
        ...
        if(count($error) == 0) {
          ...
          mysqli_query($conexao, "UPDATE parceiro SET titulo='$titulo', imagem='$nome_imagem', link='$link', status='$status' WHERE id='$id'");
        ...
        echo "<script>alert('Dados atualizados!');</script>";
          mysqli_query($conexao, "UPDATE parceiro SET titulo='$titulo', link='$link', status='$status' WHERE id='$id'");
          header('Location: ?link=parceiros'); 
